...
navLink.forEach((e) => {
  e.addEventListener('click', (x) => {
    ...
    for (let sect of section) {
      ...
      if (
        sect.getAttribute('data-section') === e.getAttribute('data-section')
      ) {
  ...
  fetch('https://countriesnow.space/api/v0.1/countries')
    ...
    .then((posts) => {
      for (let post of posts.data) {
        ...
        let countryContent = `
            <span onclick="getData('${post.iso2}', this.textContent, '${map
          .join('')
          .split(',')}')">${post.country}</span>`;
...
function getTime(is2, cty) {
  ...
  for (let link of navLink) {
    if (
      ...
      document.getElementById('prayer-times').getAttribute('data-section')
    ) {
      ...
      navigator.geolocation.getCurrentPosition(
        (location) => {
          ...
          fetch(
            ...
            .then((prayer) => {
              let dateData =
                prayer.data[date.getMonth() + 1][date.getDay() + 1];
